<template>
  <div class="my-form-row">
    ...
    <div style="display: inline-block;">
      <div v-for="(step,index) in steps" :key="step.id" class="my-form-row">
...
<script>
  import eventIndicatorsTree from './event_indicators_tree';
  export default {
    components: {
    ...
    methods: {
      ...
      createAlias(node){
        ...
        var alias = '';
        alias = node.name_array.join("_");
        if (type == 'METRIC') {
          alias += '_' + node.agg_name;
        }
        node.alias = alias ;
      },
      add_main_step() {
