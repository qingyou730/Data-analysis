    ...
    <tr v-for="(node,index) in children" :key="node.id">
      ...
      <td :class="['leaf',children.length==1?'one-leaf':'']">
        <el-select @change="changeGroupType(node)" v-model="node.group_type" style="width: 100px;">
          <el-option label="逻辑组" value="logic_group"></el-option>
          <el-option label="属性" value="attr"></el-option>
        </el-select>
        ...
        <span v-if="node.group_type=='attr'" >
          <!--属性选择联级菜单-->
          <el-cascader
            :ref="'node_attr_' + node.id"
            @change="change_value($event,node)"
            filterable
            v-model="node.value_array"
          ...
          <el-select  style="width: 150px;" v-model="node.range_attr" placeholder="请选择" v-if="node.need_op   && attr_type == 'OBJECT_RANGE' &&  options.range_attr_field_type_map.hasOwnProperty(node.field_type)">
            <el-option v-for="item in options.range_attr_field_type_map[node.field_type]" :key="item.value" :label="item.name" :value="item.value"></el-option>
          </el-select>
            ...
            <template v-if="node.need_op">
              ...
              <template v-if="node.field_type == 'LIST_MAP'">
                ...
                <template  v-if="node.box_type=='PHRASES'  ">
                  <el-select style="width: 400px;" :collapse-tags="options.list_value.hasOwnProperty(node.value)"  v-model="node.in"  clearable multiple filterable allow-create default-first-option placeholder="请选择">
                    <el-option v-if="options.list_value.hasOwnProperty(node.value)" v-for="(item,index) in options.list_value[node.value]" :key="index" :label="item.name" :value="item.value"></el-option>
                  </el-select>
                ...
                <template  v-if="node.box_type=='RANGE'  ">
                  ...
                  <el-input  v-model="node.right" placeholder="请输入最大值" style="width: 200px;"> <template slot="suffix" ><span class="input-suffix">个</span></template></el-input>
                </template>
                <!-- 区间 end-->
              </template>
              <!--装备武者走特殊逻辑 end-->
              ...
              <template v-else>
                <!-- PHRASE 单个输入框 -->
                <template  v-if="node.box_type=='PHRASE'">
                  <el-date-picker value-format="yyyy-MM-dd HH:mm:ss" v-if="node.field_type=='DATE'" v-model="node.left" type="datetime"  placeholder="选择日期时间"> </el-date-picker>
                  <template v-else>
                    ...
                    <template v-else>
                      <el-select  style="width: 350px;" v-model="node.left" placeholder="请选择"  allow-create>
                        <el-option v-for="(item,index) in options.list_value[node.value]" :key="index" :label="item.name" :value="item.value"></el-option>
                      </el-select>
                    </template>
                  </template>
                </template>
                ...
                <template  v-if="node.box_type=='RANGE'  ">
                  <template  v-if="node.field_type=='DATE'  ">
                    <el-date-picker value-format="yyyy-MM-dd HH:mm:ss" v-model="node.left" type="datetime"  placeholder="选择日期时间"> </el-date-picker>
                    <el-date-picker value-format="yyyy-MM-dd HH:mm:ss" v-model="node.right" type="datetime"  placeholder="选择日期时间"> </el-date-picker>
                  </template>
                  <template  v-else >
                    <el-input  v-model="node.left"  placeholder="请输入最小值" style="width: 200px;"></el-input>
        ...
        <!--递归渲染逻辑组下的节点-->
        <user-where-tree :attr_type="attr_type" :url_list=url_list :parent="node" v-if="node.group_type=='logic_group'"
          :options="options" :children="node.children" :logic_op="node.logic_op" :children_num="node.children.length" :add_children="add_children" >
        </user-where-tree>
...
<script>
  ...
  export default {
    name: 'UserWhereTree',
    props:['add_children','children','logic_op','children_num','options','parent','url_list','attr_type'] ,
    components: {
      EquipmentTree
    },
    data() {
    ...
    methods: {
      ...
      change_value(value, node, ) {
        ...
        node.real_value = finnal_option.hasOwnProperty('real_value') ?  finnal_option.real_value : ''  ;
        node.parent_id  = finnal_option.hasOwnProperty('parent_id') ? finnal_option.parent_id : ''  ;
