    ...
    <div>
        <el-row style="margin-top: 10px">
                  ...
                  <tbody>
                    <template v-for="(item1, index) in tableData">
                      <tr :key="index">
                        ...
                        <td style="cursor: pointer;" @click="formatSql(item1.sql)"><pre style="white-space: nowrap;text-overflow: ellipsis;font-family: inherit;font-size:14px;overflow: hidden;width:40%">
                        ...
                        <td style="padding: 6px 0;">
                          <el-row>
                            <!-- 查看成员 -->
                            <el-button v-if="$access('/hive/sqllab/submit')" type="text" size="mini" @click="linkAddTabs(item1)"
                            ...
                            <el-button v-if="$access('/hive/sqllab/submit')" type="text" size="mini"
                              ...
                              <a  target="_blank" :href="$store.state.api_url.job.yarn_log +'?job_id='+item1.id">yarn日志</a></el-button>
                            <a v-if="$access('/hive/mlist/delete')&&item1.status=='SUCCESS'"
                            ...
                            <el-button v-if="$access('/hive/mlist/delete')&&item1.status!='SUCCESS'" type="text" size="mini"
                              style="box-sizing: border-box; font-size: 12px; color:#ccc;">下载</el-button>
                          </el-row>
                        </td>
                      </tr>
                    </template>
                ...
                <el-pagination  @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage"
                  :page-sizes="[10, 20, 30, 50, 100]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total">
                </el-pagination>
...
export default {
    ...
    methods: {
      ...
      searchType(type){
        ...
        getData() {
            this.loading = true;
            let requestUrl = "/mmp/sql/sql_list";
            let data = {
        ...
        linkToEdit(item1) {
          console.log(1111);
          this.$router.push({ path: '/hive/sqllab/submit', query: { linkSql: item1.sql, name: item1.task_name,type:'download','id':item1.id}})
        },
