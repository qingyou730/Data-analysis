<template>
  <div id="fix-column">
    <el-dialog title="配置固定列" :visible.sync="visible" :close-on-click-modal="wrapper_no" :before-close="closeDialog"
      width="900px">
      <el-form style="margin-right: 38px;">
        <template v-for="(column,column_index) in column_list">
          <div style="margin:10px 30px 10px 0px" :key="column_index+'fixwra'">
            ...
            <el-form-item :key="'dimension'+column_index +d_index" :label="d_index==0 ? '枚举值：' : ''" v-if="column.type=='dimension'"
              ...
              <div style="background: #ECEEFE;padding-left:10px">
                <el-input size="small" placeholder="请输入值" v-model="d.value" style="width: 200px;"></el-input>
                <el-input v-model="d.text" style="width: 200px;" size="small" placeholder="请输入显示文字"></el-input>
            ...
            <el-form-item :key="'name'+column_index" :label="column_index==0 ? '列名称：' :''" label-width="100px">
              <div style="background: #ECEEFE;padding-left:10px">
                <el-select v-model="column.column" size="small" value-key="order" placeholder="请选择列" style="width: 200px;">
                  <el-option v-for="(indicator,indicator_index) in indicators" :key="indicator_index" :label="indicator.alias"
      ...
      <el-form v-if="previewFlag">
        <template v-for="(column,column_index) in preview_column_list" v-if="column.type && column.column.hasOwnProperty('alias')">
...
<script>
  ...
  export default {
    props: ['value', 'id', 'folder_id', 'indicators', 'task_field', 'fixed_column_str', "fixColumnParam",
      "changeShowfixFlag", "changeRigthKey","cancelPublic"
    ],
    data() {
      return {
        visible: true,
    ...
    methods: {
      ...
      save_fixed_column: function() {
        ...
        var url = this.$store.state.api_url.report.edit_fixed_column;
