<template>
  <div  class="model-fix-column">
    ...
    <el-form style="display: inline-block;">
      <template v-for="(column,column_index) in column_list" v-if="column.type && column.column">
        <el-form-item :key="'show'+column_index" :label="column.column + ':'" label-width="212px" >

          <!-- 时间类型 -->
          <template v-if="column.type=='time'">
            <el-select v-if="column.type == 'time'" v-model="column.time_type" style="width: 150px" 
            ...
            <template v-if="column.type=='time' && column.time_type=='STATIC' ">
              <template v-if="column.time_granularity=='DATE'">
                ...
                <el-date-picker v-if="column.op!='BETWEEN'"  v-model="column.value" style="width: 150px;">
            ...
            <el-popover placement="bottom" @hide="resetInput" @show="initInput(column)" width="100" trigger="click"
              ...
              <el-row v-if="item.op == column.op" :class="item.id==column.time_id ? 'checked-drop' : '' " type="flex"
                ...
                <el-input style="width: 50px;height: 10px;" @input="inputChange($event,item,column)" 
          ...
          <template v-if="(column.type=='number' || column.type=='string' || column.type=='date')">
            <el-select v-if="column.column && column.type" v-model="column.op" 
            ...
            <template v-if="column.type=='date'">
              ...
              <template v-if="column.op == 'BETWEEN'">
                <el-date-picker v-model="column.start" value-format="yyyy-MM-dd" style="width: 200px;" >
                </el-date-picker> ~
                <el-date-picker v-model="column.end" value-format="yyyy-MM-dd" style="width: 200px;" >
                </el-date-picker>
              </template>
            </template>
