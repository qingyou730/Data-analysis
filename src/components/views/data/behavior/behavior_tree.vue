<template>
	<table v-show="step_children.length>0" :class="['table_list_'+countRow(step_children),'table-2']">
		<tr v-for="(node,index) in step_children" :key="index">
			<td :style="'padding-top:'+((countRow(step_children)*46 - 30)/2)+'px;'" v-if="index==0" v-show="step_children.length>1" :class="['branch','row_'+countRow(step_children)]" :rowspan="step_children.length">
			...
			<td class="leaf">
				...
				<span v-if="node.step_type=='event'">
					...
					<el-cascader :ref="'event_node_' + node.id" @change="change_event($event,node)" filterable v-model="node.value_array"
					 :options="parent.options.event_list" :props="cascader_props">
					</el-cascader>
					...
					<template v-if="node.need_op">
						...
						<template v-if="node.box_type=='PHRASE'">
							<el-date-picker value-format="yyyy-MM-dd HH:mm:ss" v-if="node.field_type=='DATE'" v-model="node.left" type="datetime"
						...
						<template v-if="node.box_type=='PHRASES'  ">
							<el-select style="width: 400px;" collapse-tags v-model="node.in" clearable multiple filterable allow-create
				...
				<behaviorTree :attr_type="attr_type" :url_list="url_list" :parent="node" :step_logic="node.step_logic"
...
<script>
	export default {
		name: 'behaviorTree',
		props: ['url_list', 'options', 'parent', 'step_children', 'step_logic', 'add_step', 'attr_type'],
		data() {
			return {
