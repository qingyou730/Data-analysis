<template>
  <div class="custom-rewrite-file">
    <el-dialog title="移动到文件夹" :visible.sync="value" :close-on-click-modal="wrapper_no" :before-close="close" width="30%">
      <div style="margin:auto">
        <el-input placeholder="关键字搜索" style="width:60%;margin:0 20%" suffix-icon="el-icon-search" @input='searchNeedfile($event)'
          v-model="file_name">
        </el-input>
      </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      submit() {
        if (!this.file_name) {
          this.$error("请选择移动到的文件夹");
          return;
        }
        let url = this.$store.state.api_url.dynamicDroup.move_dynamic_group;
        let data = {
