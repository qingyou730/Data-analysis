  ...
  <div>
    <div style="display: inline-block;width: 100%;">
      <span style="margin:0 10px 0 9px;">群组列表</span>
        <el-input @input="selectOne(true)" v-model="q" placeholder="输入关键字搜索" clearable suffix-icon="el-icon-search"
      ...
      <router-link to="/data/group/add" v-if="$access('/data/group/add')">
        <el-button @click="countAdd" type="primary" icon="el-icon-plus" class="add-btn"></el-button>
      </router-link>
    </div>
      ...
      <tbody>
        <template v-for="(item, index) in groundList">
          <tr :key="index">
            ...
            <td>
              {{  $store.state.user_list.hasOwnProperty( item.create_role ) ? $store.state.user_list[item.create_role ] :  item.create_role }}
            </td>
            ...
            <td>
              <el-row>
                <!-- 查看成员 -->
                <el-button
                  v-if="$access('/data/group/member') "
                  type="text"
                  size="mini"
                  @click="goMember(item.id, item.group_num)"
                  :displayName="item.id"
                  id="searchMember"
                ...
                >刪除</el-button>
                  ...
                  <el-button
                    v-if="$access('/data/group/convert')"
                    type="text"
                    size="mini"
                    @click="
                      groupTrans = true
...
export default {
  data() {
    return {
      ...
      pageSize: 10,
      groundList: [],
      selectList: [{ cascade: false, name: "角色", value: "role" }],
      value: "角色",
      dialogVisible: false,
      groupTrans: false,
      delId: "",
  ...
  methods: {
    closeTransDialog:function(){
      this.$count('GROUP','TRAHSLATION_GROUP',{'group_id':this.transId,type:0});//打点统计
    ...
    lookId(item) {
      ...
      this.$confirm("此操作将永久删除该数据,不可恢复, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$count('GROUP','DROP_GROUP',{"group_id": item.id ,type:1});//打点统计
          this.delById();
        })
        .catch(() => {
          this.$count('GROUP','DROP_GROUP',{"group_id": item.id ,type:0});//打点统计
          console.log("删除取消");
        });
    },
    delById() {
    ...
    groundTrans() {
      ...
      this.$count('GROUP','TRAHSLATION_GROUP',{'group_id':this.transId,type:1});//打点统计
      this.axios
